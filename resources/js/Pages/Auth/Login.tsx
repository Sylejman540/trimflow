...
export default function Login({ status, canResetPassword }: { status?: string, canResetPassword: boolean }) {
    ...
    return (
        <div className="flex min-h-screen w-full bg-white font-sans selection:bg-slate-100">
            ...
            {/* --- RIGHT SIDE: Minimalist Form (Full width on mobile) --- */}
            <div className="w-full lg:w-1/2 flex flex-col items-center justify-start lg:justify-center px-6 sm:px-12 md:px-20 py-12 bg-white relative">
                ...
                <div className="w-full max-w-[380px]">
                    <div className="mb-10 md:mb-12">
                        <h2 className="text-3xl md:text-4xl font-light tracking-tight text-slate-900 mb-3">Sign in</h2>
                        <p className="text-slate-400 font-light text-sm">Enter your studio credentials to continue.</p>
                    </div>

                    {status && (
                        <div className="mb-6 text-[10px] font-bold uppercase tracking-widest text-slate-600 bg-slate-50 p-4 border border-slate-100">
                            {status}
                        </div>
                    )}
                    ...
                    <form onSubmit={submit} className="space-y-6 md:space-y-8">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                required
                            />
                            {errors.email && <p className="text-[10px] text-red-500 font-medium uppercase tracking-wider mt-1">{errors.email}</p>}
                        </div>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                {canResetPassword && (
                                    <Link href={route('password.request')} className="text-[9px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-900 transition-colors">
                                        Forgot?
                                    </Link>
                                )}
                            </div>
                            ...
                            {errors.password && <p className="text-[10px] text-red-500 font-medium uppercase tracking-wider mt-1">{errors.password}</p>}
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            />
                            <label htmlFor="remember" className="text-[10px] font-bold uppercase tracking-widest text-slate-400 cursor-pointer select-none">
                                Keep me signed in
                            </label>
                        </div>
                        ...
                        <div className="pt-4">
                            <button
                                type="submit"
                                disabled={processing}
                                className="group relative flex h-14 w-full items-center justify-center rounded-full bg-slate-900 text-[10px] font-bold tracking-[0.2em] text-white transition-all hover:bg-slate-800 active:scale-[0.98] disabled:opacity-50 shadow-lg shadow-slate-100"
                            >
                        ...
                        <p className="text-center text-[10px] text-slate-400 font-bold uppercase tracking-[0.1em] pt-6">
                            ...
                            <Link href={route('register')} className="text-slate-900 hover:underline underline-offset-4">
                                Join the network
                            </Link>
                        </p>
                    </form>
