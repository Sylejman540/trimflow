            ...
            <>
                <div className={cn(
                    ...
                    variant === 'danger'
                        ...
                        : active ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-400 group-hover:bg-slate-200 group-hover:text-slate-700'
                )}>
                    <Icon className="h-4 w-4" />
                </div>
                {!isCollapsed && (
                    <div className="flex-1 min-w-0">
                        <p className={cn('text-sm font-semibold leading-tight', active ? 'text-amber-700' : 'text-slate-700')}>{label}</p>
        ...
        <div className="flex h-[100dvh] overflow-hidden bg-white font-sans text-slate-900">
            ...
            <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-white border-t border-slate-200 flex items-stretch">
                ...
                {walkin && (
                    <button
                        onClick={() => setWalkinOpen(true)}
                        className="flex-1 flex flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-bold text-amber-600"
                    >
